...
def set_auth_cookies(
    ...
    logged_in_cookie_setting = cookie_settings.copy()
    logged_in_cookie_setting["httponly"] = False
    response.set_cookie("logged_in", "true", **logged_in_cookie_setting)
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if refresh_res.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                ...
                refresh_res.data["message"] = "Access tokens refresh successfully."
            else:
